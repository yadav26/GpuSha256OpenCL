...
__kernel void helloworld(__global unsigned char* in, int inlen, __global unsigned char* out)
{
...
__kernel void sha256_nonce(__global char* input, int inlen, int startIndex, __global unsigned char* out)
{
	unsigned int m_len = 0;
	unsigned int m_tot_len = 0;
...
*/
	...
	int magic_number = startIndex + num;
	char buffmn[64] = { '\0' };
//	sprintf(buffmn, "%x",  magic_number);
	printf("%s", buffmn);
	//sprintf(buffmn, "%s%x", magic_number);
